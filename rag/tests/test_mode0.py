...
def test_TEST_015_retrieve_missing_params_mode0(mk_app_client,payload):
    _, client = mk_app_client(mode=0)
...
# ------------ test get user files -------------- #
def test_TEST_018_get_user_files_ok(mk_app_client):
    _, client = mk_app_client()
...
def test_TEST_019_get_user_files_missing_user_id(mk_app_client):
    _, client = mk_app_client()
    r = client.get("/api/user_files") 
    assert r.status_code == 400
    j = r.get_json()
    assert isinstance(j, dict)
    assert j["error"] is True
