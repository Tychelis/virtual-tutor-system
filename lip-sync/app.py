...
async def offer(request):
    ...
    # 注释掉 codec preferences 设置，避免 aiortc 的方向设置错误
    ...
    # 预填充静音音频帧，确保首次连接时音频track可以立即工作
    # 这解决了首次连接时不出声的问题
    async def prefill_audio():
        # 等待一小段时间，确保worker thread有机会启动
        await asyncio.sleep(0.15)
        audio_track = player.audio
        # 预填充15个静音帧（约300ms），确保音频track在首次使用时已经准备好
        # 这可以避免首次连接时音频丢失的问题
        prefill_count = 0
        for i in range(15):
            silence = np.zeros(320, dtype=np.int16)  # 20ms at 16kHz
            silent_frame = AudioFrame(format='s16', layout='mono', samples=320)
            silent_frame.planes[0].update(silence.tobytes())
            silent_frame.sample_rate = 16000
            try:
                await audio_track._queue.put((silent_frame, None))
                prefill_count += 1
            except Exception as e:
                logger.debug(f"Failed to prefill audio frame {i}: {e}")
                break
        logger.debug(f"Prefilled {prefill_count} silent audio frames for session {sessionid}")
    
    # 在后台任务中预填充音频帧
    asyncio.create_task(prefill_audio())
    ...
    try:
        ...
        if sessionid not in nerfreals:
            ...
            return web.Response(
                content_type="application/json",
                text=json.dumps(
                    {"code": -1, "msg": f"Session {sessionid} not found or expired"}
                ),
                status=404
            )
        ...
        if nerfreals[sessionid] is None:
            logger.warning(f'Session {sessionid} is still initializing')
            return web.Response(
                content_type="application/json",
                text=json.dumps(
                    {"code": -1, "msg": f"Session {sessionid} is still initializing"}
                ),
                status=503
            )
...
async def interrupt_talk(request):
    try:
        ...
        if sessionid not in nerfreals or nerfreals[sessionid] is None:
            logger.warning(f'Session {sessionid} not found or not initialized')
            return web.Response(
                content_type="application/json",
                text=json.dumps(
                    {"code": -1, "msg": f"Session {sessionid} not found or expired"}
